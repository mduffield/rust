...
fn spawn_process_os(cfg: ProcessConfig, in_fd: c_int, out_fd: c_int, err_fd: c_int)
    ...
    unsafe {
        ...
        let mut cmd_str = make_command_line(cfg.program, cfg.args);
        ...
        with_envp(cfg.env, |envp| {
            with_dirp(cfg.cwd, |cwdp| {
                cmd_str.with_mut_ref(|cmdp| {
                    let created = CreateProcessA(ptr::null(),
                                                 mem::transmute(cmdp),
                                                 ptr::mut_null(),
                                                 ptr::mut_null(),
                                                 TRUE,
...
fn make_command_line(prog: &CString, args: &[CString]) -> CString {
    ...
    return cmd.as_slice().to_c_str();

    fn append_arg(cmd: &mut Vec<u8>, arg: &CString) {
        let arg = arg.as_bytes_no_nul();
        let quote = arg.iter().any(|c| *c as char == ' ' || *c as char == '\t');
        if quote {
            cmd.push('\"' as u8);
        }
...
#[cfg(windows)]
fn with_dirp<T>(d: Option<&CString>, cb: |*libc::c_char| -> T) -> T {
    match d {
      Some(dir) => dir.with_ref(|buf| cb(buf)),
      None => cb(ptr::null())
    }
}
...
mod tests {
    ...
    fn test_make_command_line() {
        ...
        fn test_wrapper(prog: &str, args: &[&str]) -> ~str {
            str::from_utf8_lossy(
                super::make_command_line(&prog.to_c_str(),
                                         args.iter()
                                             .map(|a| a.to_c_str())
                                             .collect::<Vec<CString>>()
                                             .as_slice())
                    .as_bytes_no_nul()).to_owned()
        }
