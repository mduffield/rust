...
impl Command {
    ...
    pub fn output(&self) -> IoResult<ProcessOutput> {
        self.spawn().map(|mut p| p.wait_with_output())
    }
...
mod tests {
    ...
    iotest!(fn test_wait_with_output_once() {
        let mut prog = Command::new("echo").arg("hello").spawn().unwrap();
    ...
    iotest!(fn test_keep_current_working_dir() {
        use os;
        let mut prog = pwd_cmd().spawn().unwrap();
    ...
    iotest!(fn test_change_working_directory() {
        ...
        let mut prog = pwd_cmd().cwd(&parent_dir).spawn().unwrap();
    ...
    iotest!(fn test_add_to_env() {
        ...
        let mut prog = env_cmd().env(new_env).spawn().unwrap();
